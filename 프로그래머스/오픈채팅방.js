/*
카카오톡 오픈채팅방에서는 친구가 아닌 사람들과 대화를 할 수 있는데, 본래 닉네임이 아닌 가상의 닉네임을 사용하여 
채팅방에 들어갈 수 있다.
...
예를 들어, 채팅방에 "Muzi"와 "Prodo"라는 닉네임을 사용하는 사람이 순서대로 들어오면 채팅방에는 
다음과 같이 메시지가 출력된다.
...
Muzi가 나간후 다시 들어올 때, Prodo 라는 닉네임으로 들어올 경우 기존에 채팅방에 남아있던 
Muzi도 Prodo로 다음과 같이 변경된다.
...
function solution(record) {
  ...
  for(info of record){
    let infoSplit = info.split(" ");
    if(infoSplit[0] == 'Enter'){
      answer.push(`${user[infoSplit[1]]}님이 들어왔습니다.`);
    }else if(infoSplit[0] == 'Leave'){
      answer.push(`${user[infoSplit[1]]}님이 나갔습니다.`);
    };
  };
  return answer;
}
